	...
	LongIntADT result;
	ListADT<int> resList = result.l;

	
	this->node = this->l.getLast();
	Q.node = Q.l.getLast();
	
	int addtmp = this->l.getVal(this->node) + Q.l.getVal(Q.node);
	int carry = overflow(addtmp);
	
	resList.insertLeft(addtmp);

	
	while (!this->l.isFirst(this->node) && !Q.l.isFirst(Q.node))
	{
	
		this->node = this->l.nextLeft(this->node);
		Q.node = Q.l.nextLeft(Q.node);
	
		addtmp = carry + this->l.getVal(this->node) + Q.l.getVal(Q.node);
		carry = overflow(addtmp);
		resList.insertLeft(addtmp);	
